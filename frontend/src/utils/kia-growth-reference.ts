...
export interface KiaGrowthReferenceRow {
  ageInMonths: number;
  weightMinus3Sd: number;
  weightMinus2Sd: number;
  weightMinus1Sd: number;
  weightMedian: number;
  weightPlus1Sd: number;
  weightPlus2Sd: number;
  weightPlus3Sd: number;
  heightMinus3Sd: number;
  heightMinus2Sd: number;
  heightMinus1Sd: number;
  heightMedian: number;
  heightPlus1Sd: number;
  heightPlus2Sd: number;
  heightPlus3Sd: number;
}
...
const interpolateFromAnchors = (anchors: Array<[number, number]>, ageInMonths: number) => {
...
const scaleAnchors: Record<
  GenderReference,
  {
    weightMedian: Array<[number, number]>;
    weightMinus2Sd: Array<[number, number]>;
    heightMedian: Array<[number, number]>;
    heightMinus2Sd: Array<[number, number]>;
  }
> = {
  MALE: {
    weightMedian: [
...
const KIA_2024_IDEAL_RANGES_0_24: Record<
  GenderReference,
  {
    weightMin: number[];
    weightMax: number[];
...
export const buildKiaGrowthReference = (gender: GenderReference, maxAgeInMonths = MAX_AGE_MONTHS): KiaGrowthReferenceRow[] => {
  const safeMaxAge = Math.max(0, Math.min(MAX_AGE_MONTHS, Math.floor(maxAgeInMonths)));
  return Array.from({ length: safeMaxAge + 1 }).map((_, ageInMonths) => {
    ...
    const weightMedian = useKiaMonthlyRange
      ? weightMinus2Sd + (2 * ((weightPlus1FromKia ?? weightMinus2Sd) - weightMinus2Sd)) / 3
    ...
    const weightPlus1Sd = useKiaMonthlyRange ? (weightPlus1FromKia ?? weightMedian + weightSd) : weightMedian + weightSd;
    ...
    const heightMedian = useKiaMonthlyRange
      ? heightMinus2Sd + (2 * ((heightPlus1FromKia ?? heightMinus2Sd) - heightMinus2Sd)) / 3
    ...
    const heightPlus1Sd = useKiaMonthlyRange ? (heightPlus1FromKia ?? heightMedian + heightSd) : heightMedian + heightSd;
